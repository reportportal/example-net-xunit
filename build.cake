...
Task("Connect-ReportPortal")
	...
	.Does(() =>
{
	CurlDownloadFile(new Uri("https://github.com/reportportal/agent-net-xunit/releases/download/1.2.0/reportportal-1.2.0-net452.zip"), new CurlDownloadSettings{FollowRedirects = true});

	DirectoryPath exePath = Context.Tools.Resolve("xunit.console.exe").GetDirectory();

	DeleteFiles(exePath + "/ReportPortal*");
	Unzip("reportportal-1.2.0-net452.zip", exePath);
